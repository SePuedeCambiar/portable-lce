...
bool m_autosave;
    ...
    // --- OPTIMIZACIÓN PARA 512MB RAM ---
    // Reducimos la reserva de memoria base. 512MB era demasiado para un PC con 8GB 
    // si queremos estabilidad. Bajamos a 128MB, que es suficiente para la mayoría de mundos.
#if defined(_LARGE_WORLDS)
static constexpr std::size_t MAX_SAVE_SIZE = 128u * 1024u * 1024u; 
#else
...
virtual std::string getFilename();
    ...
    // --- CORRECCIÓN DE FUGAS (Eliminación de punteros a vectores) ---
    // Cambiamos std::vector<FileEntry*>* por std::vector<FileEntry*>
    // Esto evita que se cree un vector con 'new' que nunca se borra.
